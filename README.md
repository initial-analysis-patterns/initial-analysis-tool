# Initial Event Log Analysis — Interactive Tool
...
environment. It accompanies a paper submitted to ICPM 2027. The tool lets you
load an event log and apply the patterns to it — inspecting attributes and
...
## What is marimo (in one paragraph)?
...
always consistent. You do not need to read or write code to use this tool — you
...
A few are easy to overlook: **`vegafusion`** and **`vl-convert-python`** (used
...
There are two ways to open it. **For reviewing, use app mode (the first).**

### Option A — App mode (recommended)

```bash
marimo run exploratory-tool.py
```

Opens the tool as a clean web app (code hidden), by default at
`http://localhost:2718`. If the tab does not open automatically, copy the URL
printed in the terminal.

### Option B — Editable mode (to inspect the code)
...
- **Supported formats:** `.xes` and `.csv`. For XES the standard attributes
  (`case:concept:name`, `concept:name`, `time:timestamp`) are used by default;
...
- **Where to get logs:** the public benchmark logs used in the paper (RTFM,
  Sepsis, and the BPI Challenge logs) are available from the IEEE Task Force on
  Process Mining: <https://www.tf-pm.org/resources/logs>.

---

## Using the tool

After you select a log, the analysis is organized top to bottom into sections;
...
- **Log Initialization** — set a timezone and (optionally) normalize timestamp
  ...
  initialization did, alongside timestamp-format and granularity analyses.
...
- **Consistency check** — check a user-specified rule against the log; a Sepsis
  SIRS example is pre-filled and can be run as is.
